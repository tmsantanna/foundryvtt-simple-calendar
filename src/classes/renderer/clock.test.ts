...
describe("Renderer Clock Class Tests", () => {
    let tCal: Calendar;
    ...
    test("Render", () => {
        let html = Clock.Render(tCal);
        expect(html).toContain('<div class="fsc-current-time">00:00:00</div>');
        expect(html).not.toContain('fsc-animate"');

        //@ts-ignore
        tCal.timeKeeper.status = TimeKeeperStatus.Started;
        html = Clock.Render(tCal);
        expect(html).toContain('<div class="fsc-current-time">00:00:00</div>');
        expect(html).toContain('fsc-animate"');

        html = Clock.Render(tCal, { id: "", theme: "light" });
        expect(html).toContain("simple-calendar light");
    });

    test("Render Time", () => {
        expect(Clock.RenderTime(tCal)).toContain('<div class="fsc-current-time">00:00:00</div>');
    });

    test("Activate Listeners", () => {
        const clockElm = document.createElement("div");
        jest.spyOn(document, "getElementById").mockReturnValue(clockElm);
        jest.spyOn(tCal.timeKeeper, "registerUpdateListener").mockImplementation(() => {});

        Clock.ActivateListeners("");
        expect(tCal.timeKeeper.registerUpdateListener).toHaveBeenCalledTimes(1);
    });
    ...
    test("Update Listener", () => {
        ...
        jest.spyOn(document, "getElementById").mockReturnValue(clockElm);
        jest.spyOn(clockElm, "querySelector").mockReturnValueOnce(optionsElm).mockReturnValue(time);

        optionsElm.value = "{}";

        Clock.UpdateListener("", TimeKeeperStatus.Started);
        expect(time.classList.contains(TimeKeeperStatus.Started));
    });
});
